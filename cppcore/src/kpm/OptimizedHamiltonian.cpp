...
template<class scalar_t>
void OptimizedHamiltonian<scalar_t>::populate(Stats& s, int num_moments, Config const& c) const {
    ...
    s.opt_nnz = num_nonzeros(num_moments, c.algorithm.optimal_size);
    s.vec = num_vec_elements(num_moments, /*optimal_size*/false);
    s.opt_vec = num_vec_elements(num_moments, c.algorithm.optimal_size);
